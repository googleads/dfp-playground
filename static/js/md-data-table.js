...
/**
 * Immediately invoked function of the Material Design data table.
...
(function(window, angular) {
  ...
  angular.module('md.table.templates', [
    'md-table-pagination.html', 'md-table-progress.html', 'arrow-up.svg',
    'navigate-before.svg', 'navigate-first.svg', 'navigate-last.svg',
    'navigate-next.svg'
  ]);

  angular.module('md-table-pagination.html', []).run([
    '$templateCache',
    function($templateCache) {
      $templateCache.put(
          'md-table-pagination.html',
  ...
  angular.module('md-table-progress.html', []).run([
    '$templateCache',
    function($templateCache) {
      $templateCache.put(
  ...
  angular.module('arrow-up.svg', []).run([
    '$templateCache',
    function($templateCache) {
      $templateCache.put(
          'arrow-up.svg',
  ...
  angular.module('navigate-before.svg', []).run([
    '$templateCache',
    function($templateCache) {
      $templateCache.put(
          'navigate-before.svg',
  ...
  angular.module('navigate-first.svg', []).run([
    '$templateCache',
    function($templateCache) {
      $templateCache.put(
          'navigate-first.svg',
  ...
  angular.module('navigate-last.svg', []).run([
    '$templateCache',
    function($templateCache) {
      $templateCache.put(
          'navigate-last.svg',
  ...
  angular.module('navigate-next.svg', []).run([
    '$templateCache',
    function($templateCache) {
      $templateCache.put(
          'navigate-next.svg',
  ...
  function mdBody() {
    ...
    return {compile: compile, restrict: 'A'};
  }
  ...
  function mdCell() {
    function compile(tElement) {
      ...
      if (select.length) {
        select.addClass('md-table-select')
            .attr('md-container-class', 'md-table-select');
      }
    ...
    function postLink(scope, element, attrs, ctrls) {
      ...
      function getIndex() {
        return Array.prototype.indexOf.call(
            element.parent().children(), element[0]);
      }
  ...
  function mdColumn($compile, $mdUtil) {
    ...
    function postLink(scope, element, attrs, ctrls) {
      ...
      function attachSortIcon() {
        ...
        $compile(sortIcon.addClass('md-sort-icon')
                     .attr('ng-class', 'getDirection()'))(scope);
      ...
      function isActive() {
        return scope.orderBy && (headCtrl.order === scope.orderBy ||
                                 headCtrl.order === '-' + scope.orderBy);
      }

      function isNumeric() { return attrs.mdNumeric === '' || scope.numeric; }

      function setOrder() {
        scope.$applyAsync(function() {
          if (isActive()) {
            headCtrl.order = scope.getDirection() === 'md-asc' ?
                '-' + scope.orderBy :
                scope.orderBy;
          } else {
            headCtrl.order = scope.getDirection() === 'md-asc' ?
      ...
      scope.getDirection = function() {
        ...
        return attrs.mdDesc === '' || scope.$eval(attrs.mdDesc) ? 'md-desc' :
                                                                  'md-asc';
      };
    ...
    return {
      ...
      restrict: 'A',
      scope: {numeric: '=?mdNumeric', orderBy: '@?mdOrderBy'}
    };
  }
  ...
  function mdEditDialog(
      $compile, $controller, $document, $mdUtil, $q, $rootScope, $templateCache,
    ...
    function build(template, options) {
      ...
      if (options.controller) {
        controller =
            getController(options, scope, {$element: element, $scope: scope});
      } else {
      ...
      if (options.clickOutsideToClose) {
        backdrop.on('click', function() { element.remove(); });
      }
    ...
    function disableScroll(element) {
      ...
      element.on('$destroy', function() { restoreScroll(); });
    }
    ...
    function getController(options, scope, inject) {
      ...
      if (options.bindToController) {
        return $controller(
            options.controller, inject, scope[options.controllerAs]);
      } else {
    ...
    function getTemplate(options) {
      return $q(function(resolve, reject) {
        ...
        function illegalType(type) {
          reject(
              'Unexpected template value. Expected a string; received a ' +
              type + '.');
        }

        if (template) {
          return angular.isString(template) ? resolve(template) :
                                              illegalType(typeof template);
        }
        ...
        if (options.templateUrl) {
          ...
          var success = function(template) { return resolve(template); };
    ...
    function escToClose(element) {
      ...
      element.on('$destroy', function() { body.off('keyup', keyup); });
    }
    ...
    function positionDialog(element, target) {
      ...
      var getHeight = function() { return element.prop('clientHeight'); };
      ...
      var getWidth = function() { return element.prop('clientWidth'); };
      ...
      return {
        controller: [
          ...
          function($element, $q, save, $scope) {
            ...
            this.dismiss = function() { $element.remove(); };

            this.getInput = function() { return $scope.editDialog.input; };

            $scope.dismiss = this.dismiss;

            $scope.submit = function() {
              update().then(function() { $scope.dismiss(); });
            };
          }
        ],
        ...
        template: '<md-edit-dialog>' +
            ...
            '<input name="input" ng-model="model" md-autofocus placeholder="{{placeholder}} "' +
            getAttrs() + '>' +
    ...
    this.show = function(options) {
      ...
      if (!options.targetEvent) {
        return logError(
            'options.targetEvent is required to align the dialog with the table cell.');
      }
      ...
      if (options.bindToController && !options.controllerAs) {
        return logError(
            'You must define options.controllerAs when options.bindToController is true.');
      }
      ...
      for (var prop in options.resolve) {
        ...
        promises.push(
            $q.when(angular.isFunction(promise) ? promise() : promise));
      }
  ...
  mdEditDialog.$inject = [
    '$compile', '$controller', '$document', '$mdUtil', '$q', '$rootScope',
    '$templateCache', '$templateRequest', '$window'
  ];
  ...
  function mdFoot() {
    function compile(tElement) { tElement.addClass('md-foot'); }
  ...
  function mdHead($compile) {
    ...
    function postLink(scope, element, attrs, tableCtrl) {
      ...
      function attatchCheckbox() {
        element.prop('lastElementChild')
            .firstElementChild.appendChild(
                $compile(createCheckBox())(scope)[0]);
      }
      ...
      function enableRowSelection() { return tableCtrl.$$rowSelect; }
      ...
      scope.allSelected = function() {
        ...
        return rows.length &&
            rows.every(function(row) { return row.isSelected(); });
      };
      ...
      scope.$watchGroup(
          [enableRowSelection, tableCtrl.enableMultiSelect],
          function(newValue) {
    ...
    return {
      ...
      require: '^^mdTable',
      restrict: 'A',
      scope: {order: '=?mdOrder', onReorder: '=?mdOnReorder'}
    };
  }
  ...
  function mdRow() {
    ...
    function postLink(scope, element, attrs, tableCtrl) {
      ...
      function isChild(node) { return element[0].contains(node[0]); }
    ...
    return {compile: compile, require: '^^mdTable', restrict: 'A'};
  }
    ...
    function postLink(scope, element, attrs, ctrls) {
      ...
      if (tableCtrl.$$rowSelect && self.id) {
        if (tableCtrl.$$hash.has(self.id)) {
          ...
          // if the item is not a reference to the current model update the
          // reference
      ...
      function createCheckbox() {
        ...
        return angular.element('<td class="md-cell md-checkbox-cell">')
            .append($compile(checkbox)(scope));
      }

      function disableSelection() {
        Array.prototype.some.call(element.children(), function(child) {
          return child.classList.contains('md-checkbox-cell') &&
      ...
      function toggle(event) {
        scope.$applyAsync(function() { self.toggle(event); });
      }
      ...
      scope.$watch(self.isSelected, function(isSelected) {
        return isSelected ? element.addClass('md-selected') :
  ...
  function Hash() {
    ...
    this.equals = function(key, item) { return keys[key] === item; };

    this.get = function(key) { return keys[key]; };

    this.has = function(key) { return keys.hasOwnProperty(key); };

    this.purge = function(key) { delete keys[key]; };

    this.update = function(key, item) { keys[key] = item; };
  }
  ...
  function mdTable() {
    ...
    function Controller($attrs, $element, $q, $scope) {
      ...
      function enableRowSelection() {
        ...
        watchListener =
            $scope.$watchCollection('$mdTable.selected', function(selected) {
      ...
      self.getBodyRows = function() {
        return Array.prototype.reduce.call(
            $element.prop('tBodies'), function(result, tbody) {
      ...
      self.waitingOnPromise = function() { return !!queue.length; };
      ...
      self.queuePromise = function(promise) {
        ...
        if (queue.push(
                angular.isArray(promise) ? $q.all(promise) :
                                           $q.when(promise)) === 1) {
          resolvePromises();
        }
      };
  ...
  angular.module('md.data.table')
      .directive('mdTablePagination', mdTablePagination);

  function mdTablePagination() {
    function compile(tElement) { tElement.addClass('md-table-pagination'); }
    ...
    function Controller($attrs, $mdUtil, $scope) {
      ...
      self.min = function() {
        return isPositive(self.total) ?
            self.page * self.limit - self.limit + 1 :
            0;
      };
      ...
      self.pages = function() {
        return isPositive(self.total) ?
            Math.ceil(self.total / (isPositive(self.limit) ? self.limit : 1)) :
            1;
      };
      ...
      $scope.$watch('$pagination.limit', function(newValue, oldValue) {
        ...
        self.page = Math.floor(
            ((self.page * oldValue - oldValue) + newValue) /
            (isPositive(newValue) ? newValue : 1));
        self.onPaginationChange();
      });
  ...
  function virtualPageSelect() {
    function Controller($element, $scope) {
      ...
      function getMin(pages, total) {
        return Math.min(
            pages, isFinite(total) && isPositive(total) ? total : 1);
      }
      ...
      content.on('scroll', function() {
        if ((content.prop('clientHeight') + content.prop('scrollTop')) >=
